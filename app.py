...
def login():
    if request.form["email"] and request.form["password"] == "admin":
        ...
        token = jwt.encode({'user': request.form["email"], 'exp': datetime.utcnow() + timedelta(seconds=60)}, new_secret_key, algorithm='HS256')
        session['token'] = token
        
        # Guardar registro de ingreso en la base de datos
        log = Log(event="login", user_id=request.form["email"])
        db.session.add(log)
        db.session.commit()
...
def logout_user():
    ...
    # Guardar registro de logout en la base de datos
    log = Log(event="logout")
    db.session.add(log)
    db.session.commit()
